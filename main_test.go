	...
	testCases := []struct {
		...
		{name: "NoFilter", root: "testdata",
			...
			expected: strings.Join([]string{
				filepath.Join("testdata", "dir.log"),
				filepath.Join("testdata", "dir2", "script.sh"),
				filepath.Join("testdata", "dir3", "archive.rar"),
				filepath.Join("testdata", "dir3", "excel.xlsx"),
				filepath.Join("testdata", "dir3", "file1.log"),
				filepath.Join("testdata", "dir3", "file1.txt"),
				filepath.Join("testdata", "dir3", "file2.txt"),
				filepath.Join("testdata", "dir3", "file3.txt"),
				filepath.Join("testdata", "dir3", "image.bmp"),
				filepath.Join("testdata", "dir3", "presentation.pptx"),
				filepath.Join("testdata", "dir3", "wscript.wls"),
			}, "\n") + "\n"},
		{name: "FilterExtensionMatch", root: "testdata",
			cfg:      config{ext: ".log", size: 0, list: true},
			expected: filepath.Join("testdata", "dir.log") + "\n" + filepath.Join("testdata", "dir3", "file1.log") + "\n"},
		{name: "FilterExtensionsMatch", root: "testdata/dir3",
			cfg: config{ext: ".rar .txt .bmp", size: 0, list: true},
			expected: strings.Join([]string{
				filepath.Join("testdata", "dir3", "archive.rar"),
				filepath.Join("testdata", "dir3", "file1.txt"),
				filepath.Join("testdata", "dir3", "file2.txt"),
